...
function init() {
    ...
    var nodes = $('body');
//     initGoogleMail(nodes);


    //This function will initialize the application inside GMail
    function initGoogleMail(initNode) {
        gmailNode = $(initNode).get(0);
        gmailNode.style.paddingRight = (currentWidth) + 'px';
        ...
        //Observe third-party width change
        MutationObserver && new MutationObserver(function (records, instance) {
            for (var i=0; i<records.length; i++) {
                var record = records[i];
                if (record.attributeName === 'style') {
                    onGmailStyleChange(gmailNode.style);
                    break;
                }
            }
        }).observe(gmailNode, {
    ...
    //This function will initialize the application inside Google Contacts
    function initGoogleContacts(initNode) {
        gmailNode = initNode;
        gmailNode.style.paddingRight = (currentWidth) + 'px';
        ...
        appNode.setAttribute("style", "border-left: 1px solid #e5e5e5;"
            ...
            + "width: " + (currentWidth - 1) + "px;"
            + "position: absolute;"
            + "top: 60px;"                          //top position of the app container
            + "bottom: 0;"                          //stretch to bottom
            + "right: 0;"
            + "overflow: auto;"
            + "transition: all 0.3s ease");

        console.debug("[main] Google Contacts' content DIV found. Attaching the app to it", gmailNode);
        console.debug("[main] Attaching node to DOM", appNode);

        gmailNode.appendChild(appNode);
    }

    //third party width change watch
    function onGmailStyleChange() {
        if (gmailNode && gmailNode.style.width) {
            var width = parseInt(gmailNode.style.width);
            if (onGmailStyleChange.width !== width) {

                //if Cirrus SalesForce App is present
                if ($('#ciAppContainer')[0]) {
                    if (!gmailNode.style.boxSizing) gmailNode.style.boxSizing = 'border-box';
                    var newWidth = width - currentWidth;
                    gmailNode.style.width = newWidth + 'px';
                    onGmailStyleChange.width = newWidth;
                }
