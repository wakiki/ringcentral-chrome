...
window.telephonise = function() {
	...
	for ( var i = 0; i < elements.length; i++ ) {
		...
		if ( elements[i].nodeType == 3 ) {
			...
			if( jQuery(elements[i]).parents('.telephonised').length > 0 ){
				continue
			}
			...
			if ( p33_ctd.ignoreElements.indexOf(elements[i].parentNode.tagName.toLowerCase()) < 0 ) {
				for( var ie = 0; ie < p33_ctd.validExpressions.length; ie++ ) {
					...
					var telephoneNode = false

					if( /phone-number/.test(elements[i].parentNode.className) ){
						test = elements[i].textContent.replace(/\s/g, '')
						telephoneNode = true
					}

					if ( test || telephoneNode) {
						var telNum = '';
						if(telephoneNode){
							telNum = test
							// throw(telNum)
						}
						if ( typeof test[0] == 'string' && !telephoneNode) {
							telNum = test[0].replace(/[^0-9]/ig,'');
						}
						matchText(elements[i].parentNode, (telephoneNode ? /(?:.*)/ : p33_ctd.validExpressions[ie]), function(node, match, offset) {
							...
							if(telephoneNode){
								jQuery(node.parentNode).append(span)
							}else{
								node.parentNode.insertBefore(span, node.nextSibling);
							}

							jQuery(node.parentNode).addClass('telephonised')
...
setInterval(function(){telephonise()}, 5000)
